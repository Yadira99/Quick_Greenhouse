...
class  Media {
  ...
  public $vehiculoPath = SITE_ROOT.DS.'..'.DS.'uploads/vehiculos';


  public $errors = array();
  public $upload_errors = array(
    0 => 'Archivo subido con exito',
    1 => 'El archivo excede el tamaño maximo php.ini',
    2 => 'EL archivo excede el tamaño maximo html',
    3 => 'Archivo subido parcialmente',
    4 => 'Ningun archivo fue subido',
    6 => 'Ruta de archivo perdido',
    7 => 'Error al subir el archivo',
    8 => 'Una extension de php '
  );
 ...
 public function process(){
    ...
    elseif(empty($this->fileName) || empty($this->fileTempPath)):
      ...
      return false;
    elseif(!is_writable($this->vehiculoPath)):
      $this->errors[] = $this->vehiculoPath." Debe tener permisos de escritura!!!.";
      return false;
    elseif(file_exists($this->vehiculoPath."/".$this->fileName)):
 ...
 /*--------------------------------------------------------------*/
  public function process_media(){
    ...
    if(!is_writable($this->vehiculoPath)){
        $this->errors[] = $this->vehiculoPath." Debe tener permisos de escritura!!!.";
        return false;
      }

    if(file_exists($this->vehiculoPath."/".$this->fileName)){
      $this->errors[] = "El archivo {$this->fileName} Realmente existe.";
      return false;
    }

    if(move_uploaded_file($this->fileTempPath,$this->vehiculoPath.'/'.$this->fileName))
    {
...
/*--------------------------------------------------------------*/
   public function media_destroy($id,$file_name){
     ...
     if(delete_by_id('media',$id)){
         unlink($this->vehiculoPath.'/'.$this->fileName);
         return true;
     } else {
